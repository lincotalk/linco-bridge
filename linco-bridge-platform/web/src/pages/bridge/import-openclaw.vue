    ...
    <view class="import-page__body">
      ...
      <template v-else>
        <view class="import-page__tip card">
          ...
          <text class="import-page__tip-desc">连接成功后选择要绑定的 OpenClaw Agent。</text>
        </view>

        <CommandBlock :commands="commandText" @copied="markCopied" />

        <view v-if="connected && contexts.length" class="context card">
          <text class="context__title">选择绑定 Agent</text>
          <view
