...
- Node.js 20+（建议与仓库其他子项目保持一致）
- **完整联调必须先启动 `../server`**，因为 H5 开发时 `/api` 会代理到 `127.0.0.1:3300`
- 如果你只是想先体验开源仓库里已经提交的 UI，**可以跳过** `node scripts/generate-icons.mjs`
- 只有在你需要**重新从 AIChat Flutter 资源生成图标**时，才需要执行 `generate-icons.mjs`；该脚本依赖同级外部目录 `../../../aichat/assets/icons`
...
```bash
cd /Users/admin/Desktop/开源项目/linco-bridge/linco-bridge-platform/server
npm install
npm run start:dev
```
...
```bash
cd /Users/admin/Desktop/开源项目/linco-bridge/linco-bridge-platform/web
npm install
npm run dev:h5
```

如果你正在维护图标资源，并且本机存在 `../../../aichat/assets/icons`，可额外执行：
...
打开 `npm run dev:h5` 输出的本地 H5 地址，然后进入底部 **桥接** Tab。
...
1. 打开 **桥接**
2. 点击 **从 Codex 导入**
3. 页面会展示一组 `setupCommands`
4. 在本机终端执行页面给出的命令
5. 回到 H5 点击「检测连接」
6. 成功后点击「继续」，进入 Agent 落地页
7. 发送一条测试消息，验证聊天链路是否打通
...
- `GET /api/demo-config` 可访问
- H5 能正常打开并显示 **消息 / 桥接** 两个 Tab
- **桥接** 页能看到 `Codex / Claude Code / Hermes / OpenClaw` 四张卡片
- 进入 **从 Codex 导入** 页面后能看到 setup 命令
- 执行 `linco-connect` 命令后，H5 中「检测连接」能通过
- 进入聊天页后，发送消息能看到流式返回
- 消息结束后，返回 **消息** Tab 能看到新会话

### 5. Hermes / OpenClaw 的额外验证点

- `Hermes` 和 `OpenClaw` 不是聊天页内自由切换 Profile / Agent 的模式
- 它们需要在导入页先完成一次性绑定
- 验证时应额外检查：
  - 是否能拉到可绑定列表
  - 绑定后是否能正常进入聊天页
  - 聊天页中是否**没有**错误地暴露二次切换入口
...
如果你只想做前端纯 UI 调试，而不启动后端：

```bash
VITE_USE_REMOTE_API=false npm run dev:h5
```

如需强制 AgentChat 走 Mock：

```bash
VITE_AGENT_CHAT_SDK=mock npm run dev:h5
```
...
```text
/Users/admin/Desktop/开源项目/aichat/assets/icons
```

如果你只是想运行开源仓库里的 H5 页面，而不是重新生成图标，可以直接使用仓库中已提交的 `src/static/icons/`，**跳过这一步**。
