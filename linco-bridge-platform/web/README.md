...
- 只有在你需要**重新从 AIChat Flutter 资源生成图标**时，才需要执行 `generate-icons.mjs`

### 1. 启动后端

在 `linco-bridge-platform/server` 目录执行：
...
在 `linco-bridge-platform/web` 目录执行：
...
4. 在本机终端执行页面给出的命令
5. 回到 H5，点击 **我已复制，获取连接状态**，等待页面确认连接成功
6. 点击 **进入 Codex**，进入 Codex 聊天页
7. 如需切换项目或进入已有会话，点击右上角文件夹图标打开 **项目 / 会话** 选择器
8. 在项目列表中可直接进入已有会话，也可点击 `+` 在当前项目下新建会话
9. 发送一条测试消息，验证聊天链路是否打通
...
| Agent | 工作区选择 | Profile / Agent 绑定 | 模型 + 推理设置 | Slash 命令补全 |
|-------|-----------|---------------------|----------------|---------------|
| Codex | ✅ 会话内可切换 | — | ✅（紧凑工具栏 `项目 \| 设置 ⌄`） | ✅ `/help` 缓存 + 输入 `/` 联想 |
| Claude | ✅ | — | ✅ | ✅ |
...
### Slash 命令（Codex / Claude）

- 落地页 / 会话页进入时：先读本地缓存，再调用 `POST /api/agent-chat/:type/bridge-command`（`/help`）刷新
- 解析 `payload.items` 为命令列表，写入内存 + `localStorage`
- 输入框输入 `/` 前缀时展示联想面板，选中后填入命令（带占位符的命令自动补空格）
...
| `useSlashCommands` | `/help` 命令预加载、缓存与输入联想数据源 |
...
| `SlashCommandSuggestionPanel` | 输入框 `/` 命令联想面板 |
