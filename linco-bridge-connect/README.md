...
| `/help` | 按当前模式显示可用本地命令 |
| `/status` | 显示当前会话状态 |
| `/session` | 显示当前桥接会话绑定的 Agent Session ID；仅 Claude/Codex |
...
| `/sessions [limit]` | 列出当前项目下最近的本地 Agent 会话；仅 Claude/Codex |
| `/chats [limit]` | 列出 Codex Desktop 侧边栏中的 Chats；仅 Codex |
| `/bind <Session ID>` | 将当前未绑定的 IM 会话绑定到当前项目下已有的本地 Agent 会话；仅 Claude/Codex |
| `/bind --chat <Chat ID>` | 将当前未绑定的 IM 会话绑定到已有的 Codex Desktop Chat；仅 Codex |
| `/history [limit]` | 显示当前已绑定 Agent session 最近聊天内容，默认 10 轮；仅 Claude/Codex |
| `/history --chat <Chat ID> [limit]` | 在不绑定的前提下预览 Codex Desktop Chat 历史；仅 Codex |
| `/history-reload [limit]` | 先刷新当前 Agent 会话并重新加载本地历史，再返回最近聊天内容；`/sync-history` 为同义命令 |
...
| `/update` | 查看、升级或降级 Linco Connect；`/upgrade` 为同义命令 |
...
| `/reasoning` | 显示或切换当前 Agent 的 reasoning effort；主要用于 Claude/Codex |
| `/settings` | 查看桥接模式下可用的模型与 reasoning 设置；仅 Claude/Codex |
| `/settings apply --reasoning <effort> --model <model>` | 一次性应用 reasoning 与 model 运行时设置；仅 Claude/Codex |
| `/compact` | 触发当前 Agent 原生上下文整理；Hermes 会映射到原生命令 `/compress` |
...
前端展示命令时应按当前 `agentType` 过滤：Claude/Codex 可展示 `/session`、`/pwd`、`/cd`、`/project`、`/sessions`、`/bind`、`/history`、`/pc`；Codex 额外展示 `/chats`、`/bind --chat`、`/history --chat`；OpenClaw 展示 `/agent`；Hermes 展示 `/profile`；`/model`、`/reasoning`、`/compact`、`/remove-account` 等命令应按当前 Agent 是否支持运行时切换或上下文整理来展示。`/commands`、`/refresh` 已移除，不应再展示。除本地命令外，其他 `/xxx` 会透传给当前 Agent。部分 Agent 原生命令只适合交互式 CLI/TUI，在桥接模式下可能没有输出。
