...
const { buildPresenceEvent } = require('./presence');
const {
  isLincoMessage,
...
const {
  buildHeartbeatMessage: buildProtocolHeartbeatMessage,
  connectorKey,
  remoteSessionScope,
...
class ImConnector {
  ...
  handleChatMessage(msg, session) {
    ...
    const attachments = lincoFilesToAttachments(normalizeLincoFiles(msg));
    ...
    }, ws, session, this.config, executeAgentQuery);
  }
...
输入 /help 查看可用命令`);
  ...
  buildHeartbeatMessage(type) {
    return buildHeartbeatMessage(this.config, {
      type,
      agentType: this.agentType,
      meta: connectorLincoMeta(this, {}),
    });
...
function buildHeartbeatMessage(config, options = {}) {
  return buildProtocolHeartbeatMessage(config, {
    ...options,
