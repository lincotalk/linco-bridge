...
| Hosted demo | Signed anonymous visitor sessions isolate connections, sessions, and messages | Client- and device-scoped demo state is not a formal account, cross-device recovery, or production multi-tenant security model; **third-party forked frontends must not call the official demo API—self-host the server instead** |
